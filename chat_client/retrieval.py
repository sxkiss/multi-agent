"""
@input: json, logging, os, sqlite3, openai; chat_client.memory
@output: RAGService, SimpleVectorDB, Mem0Service, ExternalRAGService
@position: RAG layer — 向量检索、Mem0 记忆、外部知识库
...
class SimpleVectorDB:
    def __init__(self, persist_dir: str):
        ...
        self._index: dict[str, int] = {}  # P1-9: id -> index mapping for O(1) lookup
        self._lock = threading.Lock()  # 向量化后台线程与检索并发访问保护
    ...
    def load(self):
        if os.path.exists(self.persist_file):
            try:
                ...
                # P1-9: Build index on load
                self._index = {doc['id']: i for i, doc in enumerate(self.data)}
            except Exception:
                # 文件损坏（如写入中断）时备份并从空库开始，避免反复解析失败
                try:
                    os.replace(self.persist_file, self.persist_file + ".corrupt")
                except Exception:
    ...
    def add(self, documents: list[str], metadatas: list[dict], ids: list[str], embeddings: list[list[float]]):
        """P1-9: 使用 dict 索引实现 O(1) 插入/替换，避免 O(n) 全量扫描"""
        with self._lock:
            for doc, meta, doc_id, emb in zip(documents, metadatas, ids, embeddings):
                ...
                if doc_id in self._index:
                    ...
                    self.data[idx] = new_record
                else:
                    # 新增记录
                    self._index[doc_id] = len(self.data)
                    self.data.append(new_record)
            self.save()

    def search(self, query_embedding: list[float], n_results: int = 5, score: float = 0.5, where: dict | None = None, metric: str = "cosine") -> list[dict]:
        """
        ...
        with self._lock:
            ...
            if where:
                # P2-31: where 过滤放在锁内，避免并发 add 时读到共享 dict 元素
...
class RAGService:
    def __init__(
        ...
        # embedding 未配置时用占位 key 创建（调用层 get_embedding 已有异常降级），
        # 避免初始化阶段直接抛错导致整个 Agent 无法创建
        self.embedding_client = openai.OpenAI(
            api_key=self.emb_api_key or "EMPTY",
            base_url=self.emb_base_url or "https://api.openai.com/v1",
            default_headers=default_headers
        )
        # else:
    ...
    def get_embedding(self, text: str) -> list[float]:
        # P0：embedding 未配置（占位 key "EMPTY"）时短路返回，不发起注定失败的请求
        if not self.emb_api_key or not self.emb_base_url:
            logger.warning("[RAG] embedding 未配置（emb_api_key/emb_base_url 为空），跳过向量化")
            return []
        try:
            ...
            # P0：静默丢错误上下文 → 记录日志，避免排障无痕
            logger.warning("[RAG] embedding 调用失败，向量化降级为空: %s", e, exc_info=True)
            return []
    ...
    def search(self, query: str, session_id: str | None = None,score: float = 0.2, scope: str = "session", exclude_ids: list[str] | None = None,full_text:bool = True,session_history: list[dict[str, Any]] | None = None,enable_rag_judgment: bool | None = None,) -> list[str]:
        """
        从知识库中检索与查询最相关的上下文信息。
        - query: 用户查询文本
...
class ExternalRAGService:
    ...
    def __init__(
        self,
        appid: str = "ai_app_002",
        index_id: str = "main",
        rag_final_count: int = 5,
        enable_rag_judgment: bool = False,
        api_key: str = "--",
        base_url: str = "https://www.ai-assistant.cn/plugin_api/chat/openai/v3",
        model_name: str = "fast",
        default_headers: dict[str, str] | None = None
    ):
        self.appid = appid
        ...
        # P2-33: 模块级 requests.Session 共享连接池，避免裸 requests.post 无连接复用
        self._http_session = requests.Session()
        self._judgment_agent = None
    ...
    def _should_use_rag(self, user_input: str, session_history: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        """
        判断是否需要 RAG 检索（内部方法）
        - user_input: 当前用户输入
...
请以 JSON 格式返回：
{
    ...
    def search(self, query: str, score: float = 0.2, scope: str = "session", full_text: bool = True, enable_rag_judgment: bool | None = None, session_history: list[dict[str, Any]] | None = None) -> list[str]:
        ...
        if enable_rag_judgment:
            ...
            # logger.info(f"RAG Judgment: {rag_judgment}")
            # logger.info(f"Session: {session_history}")
...
class Mem0Service:
    """Mem0 记忆服务客户端，通过 HTTP API 与本地 mem0 服务通信"""
    
    def __init__(self, agent_id: str = "ai-agent", rag_final_count: int = 5, 
    ...
    def search(self, query: str, score: float = 0.1) -> list[dict]:
        ...
        try:
            resp = self.session.post(
                ...
                json={"query": query, "agent_id": self.agent_id, "top_k": 10, "threshold": score},
                timeout=10
            )
    ...
    def add_document(self, text: str, metadata: dict | None = None):
        ...
        try:
            ...
            logger.debug(f"Mem0Service add_document 异常: {e}")

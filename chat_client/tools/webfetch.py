...
def _is_private_ip(hostname: str) -> bool:
    """P1-8: Resolve hostname and reject private/internal/loopback/link-local addresses to prevent SSRF."""
    try:
        infos = socket.getaddrinfo(hostname, None, family=socket.AF_UNSPEC)
        if not infos:
            return True  # DNS failure treated as private
        ...
        return True  # DNS failure treated as private, fail closed


def _validate_url_not_ssrf(url: str) -> str | None:
    """Return None if safe, or a rejection reason string if SSRF detected."""
    try:
        parsed = urlparse(url)
    except ValueError:
        return "Invalid URL format"
    if parsed.hostname and _is_private_ip(parsed.hostname):
        return f"URL '{parsed.hostname}' resolves to a private/internal address, blocked (SSRF prevention)"
    return None
...
def WebFetch(url: str, format: str = "markdown", timeout: int | None = None, **kwargs) -> str:
    """
    ...
    # P1-8: SSRF protection - block private/internal IPs
    ssrf_block = _validate_url_not_ssrf(url)
    if ssrf_block:
        return _xml_response("error", ssrf_block)
    ...
    try:
        ...
        if not encoding or encoding.lower() == 'iso-8859-1':
            ...
            import re
            meta_charset = re.search(b'<meta.*?charset=["\']*(.+?)["\'>]', content, re.IGNORECASE)
            if meta_charset:
        ...
        return _xml_response("error", f"Request failed: {e!s}")
    except Exception as e:
        return _xml_response("error", f"An error occurred: {e!s}")
